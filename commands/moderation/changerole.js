...
module.exports = {
  ...
  async execute(interaction) {
    ...
    collector.on('collect', async i => {
      if (i.user.id !== interaction.user.id) return i.reply({ content: 'This menu is only for the user who ran the command.', flags: 64 });
      ...
      if (i.customId === 'confirm' && i.user.id === interaction.user.id) {
        ...
        for (const channelId of selections.channels) {
          ...
          if (channel && typeof channel.permissionOverwrites?.edit === 'function') {
            await channel.permissionOverwrites.edit(role, permissionChanges);
